  ...
  if (node.marks) {
    for (const mark of node.marks) {
      if (mark.type === "bold") el = <strong className="font-semibold">{el}</strong>;
        ...
        el = (
          <code className="bg-neutral-100 dark:bg-neutral-800 text-rose-600 dark:text-rose-400 rounded px-1 py-0.5 text-sm font-mono">
            {el}
          </code>
        );
    }
...
function renderNode(node: TipTapNode, index: number): ReactNode {
  switch (node.type) {
    ...
    case "heading": {
      ...
      if (level === 1)
        return (
          <h1 key={index} className={`${base} ${size}`}>
            {children}
          </h1>
        );
      if (level === 2)
        return (
          <h2 key={index} className={`${base} ${size}`}>
            {children}
          </h2>
        );
      return (
        <h3 key={index} className={`${base} ${size}`}>
          {children}
        </h3>
      );
    }
    ...
    case "bulletList":
      return (
        <ul
          key={index}
          className="list-disc pl-6 mb-4 space-y-1 text-neutral-800 dark:text-neutral-200"
        >
          {node.content?.map((n, i) => renderNode(n, i))}
        </ul>
      );
    ...
    case "codeBlock":
      return (
        <pre
          key={index}
          className="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-4 mb-4 overflow-x-auto"
        >
