# Key prompt from the note-project specification

I'm building a "Note Taking" web app.

In this app, users will be able to create and manage notes. A "note" is simply a piece of text, created via a rich text editor (TipTap).
...
I'll build the app using Next.js with Bun & TypeScript.
For styling, we'll use TailwindCSS.
...
The data will be stored as JSON in SQLite database.
I'll use Bun's built-in SQLite Client with raw SQL statements.
